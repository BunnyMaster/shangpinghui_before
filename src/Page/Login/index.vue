...
export default {
  ...
  setup() {
    ...
    const Fun = reactive({
      //用户登录
      async UserLogin() {
        const {phone, password} = Str;
        try {
          let data = {phone, password};
          (phone && password) && await User.dispatch("UserLogin", data);
          phone !== '' && password !== '' ? UseRouter.push("/") : alert("用户名或密码不能为空");
        ...
        form {
          ...
          .input-text {
            ...
            span {
              ...
              //background: url(src/assets/logo.png) no-repeat -10px -201px;
              box-sizing: border-box;
